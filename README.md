# sciwork 2026 Conference Repository

This repository holds the website for sciwork conference 2026. The site is hosted at [https://conf2026.sciwork.dev](https://conf2026.sciwork.dev) via Netlify. It is built using [Next.js](https://nextjs.org/) bootstrapped with [`create-next-app`](https://github.com/vercel/next.js/tree/canary/packages/create-next-app) and [Tailwind CSS](https://tailwindcss.com/).
...
Open [http://localhost:8000](http://localhost:8000) with your browser to see the result. The development server binds to `0.0.0.0` by default, so it can also be reached from the local network using the machine's IP address.
...
- The `main` branch is deployed to the official website [https://conf2026.sciwork.dev](https://conf2026.sciwork.dev) and [https://conf2026.netlify.app](https://conf2026.netlify.app).

- The `develop` branch is deployed to the under development [https://develop--conf2026.netlify.app](https://develop--conf2026.netlify.app).

- Other branches are deployed to `https://{branch}--conf2026.netlify.app`. Replace `/` in the branch name with `-`.

- Pull requests are deployed to `https://deploy-preview-{id}--conf2026.netlify.app`. `{id}` is the PR ID.
